...
export const RegisterView: React.FC<RegisterViewProps> = ({ onBack, onRegisterSuccess }) => {
  ...
  return (
    <div className="min-h-screen pt-24 pb-10 px-4 sm:px-6">
      ...
      <div className="max-w-md mx-auto bg-[#FFD580]/10 p-8 rounded-3xl border-2 border-[#FFD580]/30 shadow-2xl">
        <div className="space-y-6">
          ...
          <div>
            ...
            <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full bg-[#001F3F] border-2 border-[#FFD580] p-4 rounded-xl text-xl text-[#FFFDD0] focus:outline-none focus:ring-2 focus:ring-[#FFD580]" placeholder="Mínimo 6 caracteres" />
          </div>
          <div>
            <label className="block text-[#FFD580] text-lg font-bold mb-2">REPETIR CONTRASEÑA</label>
            <input type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className="w-full bg-[#001F3F] border-2 border-[#FFD580] p-4 rounded-xl text-xl text-[#FFFDD0] focus:outline-none focus:ring-2 focus:ring-[#FFD580]" placeholder="Repita su contraseña" />
          </div>
          {error && <div className="bg-red-500/20 border border-red-500 p-4 rounded-xl text-red-200 text-center font-bold">{error}</div>}
          <button onClick={handleRegister} disabled={loading} className="w-full bg-[#FFD580] text-[#001F3F] text-2xl font-bold py-4 rounded-xl shadow-lg active:scale-95 transition-transform disabled:opacity-50">
      ...
      <Modal isOpen={modalState.isOpen} title={modalState.title} message={modalState.message} onClose={modalState.onClose} />
    </div>
  );
};

      ...
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        {links.map((link, index) => (
          <a 
            ...
            className="block bg-[#FFD580] text-[#001F3F] p-6 rounded-2xl shadow-xl transform active:scale-[0.98] transition-all hover:brightness-110"
          >
            <h3 className="text-2xl sm:text-3xl font-bold mb-2 uppercase">{link.title}</h3>
            {link.description && (
              <p className="text-lg sm:text-xl opacity-90 leading-relaxed">{link.description}</p>
            )}
